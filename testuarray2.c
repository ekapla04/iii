...
{
        number *entry_p = p1; // value we are searching for

        *((bool *)p2) &= UArray2_at(a, i, j) == entry_p; // we check if OK is true AND if the value at (i,j) is equal to p1

        if ( (i == (DIM1 - 1) ) && (j == (DIM2 - 1) ) ) {
                /* we got the corner */
                *((bool *)p2) &= (*entry_p == MARKER); // we check if OK is true AND if the value we are searching for is 99
        }
...
main(int argc, char *argv[])
{
        ...
        *((number *)UArray2_at(test_array, DIM1 - 1, DIM2 - 1)) = MARKER;
        ...
        // printf("Element: %ld\n", *((number *)UArray2_at(test_array, DIM1 - 1, DIM2 - 1)));
